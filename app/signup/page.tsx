  ...
  return (
    <section className='w-full min-h-screen mt-15 flex flex-row gap-10 lg:gap-20 p-5 items-center justify-center '>
        <div className='left-sec w-full bg-[#1C1C18] p-6 rounded-lg shadow-sm shadow-[#e6d3a3] border-2 border-[#e6d3a3] gap-10 flex flex-col  '>
            ...
            <form action="/signup" method="POST" className={`flex flex-col gap-4 ${cairo.className}`}>  
                <div className='mb-4'>
                    <label htmlFor="email" className='block text-[#e6d3a3] mb-2'>البريد الإلكتروني</label>
                    <input type="email" id="email" name="email" required className=' rounded-lg bg-[#1C1C18] w-full outline-none text-[#e6d3a3] placeholder:text-[#e6d3a3] border-2 border-[#e6d3a3] p-2 placeholder:opacity-70' placeholder='مثال :   ex@gmail.com'/>
                </div>
                <div className='mb-4'>
                    <label htmlFor="name" className='block text-[#e6d3a3] mb-2'>الاسم الكامل</label>
                    <input type="text" id="name" name="name" required className=' rounded-lg bg-[#1C1C18] w-full outline-none text-[#e6d3a3] placeholder:text-[#e6d3a3] border-2 border-[#e6d3a3] p-2 placeholder:opacity-70' placeholder= ' مثال : عبدالله محمد'/>
                </div>
                <div className='mb-4'>
                    <label htmlFor="password" className='block text-[#e6d3a3] mb-2'>كلمة المرور</label>
                    <input type="password" id="password" name="password" required minLength={6} className=' rounded-lg bg-[#1C1C18] w-full outline-none text-[#e6d3a3] placeholder:text-[#e6d3a3] border-2 border-[#e6d3a3] p-2 placeholder:opacity-70' placeholder='كلمة المرور (6 أحرف على الأقل)'/>
                </div>
                <div className='mb-4'>
                    <label htmlFor="confirmPassword" className='block text-[#e6d3a3] mb-2'>تأكيد كلمة المرور</label>
                    <input type="password" id="confirmPassword" name="confirmPassword" required className=' rounded-lg bg-[#1C1C18] w-full outline-none text-[#e6d3a3] placeholder:text-[#e6d3a3] border-2 border-[#e6d3a3] p-2 '/>
                </div>
