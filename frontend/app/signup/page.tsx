    ...
    return (
        <section className="w-full min-h-screen mt-15 flex flex-row gap-10 lg:gap-20 p-5 items-center justify-center">
            <div className="left-sec w-full bg-[#1C1C18] p-6 rounded-lg shadow-sm shadow-[#e6d3a3] border-2 border-[#e6d3a3] gap-10 flex flex-col">
                ...
                <form onSubmit={form.handleSubmit(onsubmit)} className={`flex flex-col gap-4 ${cairo.className}`}>
                    ...
                    <div className="mb-4">
                        ...
                        <PhoneInput value={form.watch("studentPhone")} onChange={(val) => form.setValue("studentPhone", val)} />
                    ...
                    {/* parent phone */}
                    <div className="mb-4">
                        <label className="block text-[#e6d3a3] mb-2">رقم هاتف ولي الأمر</label>
                        <PhoneInput value={form.watch("parentPhone")} onChange={(val) => form.setValue("parentPhone", val)} />
                    ...
                    {/* Governorate */}
                    <div className="mb-4">
                        <label htmlFor="governorate" className="block text-[#e6d3a3] mb-2">المحافظة</label>
                        <GovCombo value={form.watch("governorate")} onChange={(val) => form.setValue("governorate", val)} />
                        {
                    ...
                    {/* Year & specialization */}
                    <div className="mb-4">
                        <label htmlFor="year" className="block text-[#e6d3a3] mb-2">الصف الدراسي</label>
                        <YearCombo year={form.watch("YearCombo")} specialize={form.watch("specialization")} onSpecializationChange={(val) => form.setValue("specialization", val)} onYearchange={(val) => form.setValue("YearCombo", val)} />
                        {
    ...
    return (
        <div className="flex gap-5 items-center justify-center">
            <Combobox >
                ...
                />
                <ComboboxContent className={`bg-[#1C1C18] border-2 border-[#e6d3a3] mt-1 rounded-lg shadow-sm shadow-[#e6d3a3] ${cairo.className}`}>
                    <ComboboxList>
                        {years.map((year) => (
                            <ComboboxItem
                                key={year}
...
function PhoneInput({
    ...
    const filteredCountries = useMemo(() => {
        ...
        return countries.filter((c) => {
            ...
            return c.toLowerCase().includes(q) || `+${code}`.includes(q) || code.includes(q);
        });
    }, [search, countries]);
    ...
    return (
        <div className="relative w-full" ref={dropdownRef}>
            {/* Main input row */}
            <div className={`flex items-center border-2 border-[#e6d3a3]  rounded-lg bg-[#1C1C18] overflow-visible transition-colors duration-200`}>
                ...
                >
                    <span className="font-semibold text-sm tracking-wide">{selectedCountry}</span>
            ...
            <span className={`text-[#e6d3a3]/30 text-sm tracking-wide pr-3 ${!localNumber ? 'hidden' : ''}`}>رقم الهاتف | {fullno}</span>
                        ...
                        {filteredCountries.length === 0 ? (
                            ...
                            filteredCountries.map((c) => (
                                <button
                                    ...
                                    className={`w-full text-left flex items-center justify-between px-3 py-2 text-sm cursor-pointer hover:bg-[#2a2a25] transition-colors ${c === selectedCountry ? "bg-[#2a2a25] text-[#e6d3a3]" : "text-[#e6d3a3]/80"
                                        }`}
                                >
                                    <span className="font-medium">{c}</span>
                                    <span className="text-[#e6d3a3]/50">+{getCountryCallingCode(c)}</span>
                                </button>
                            ))
                        )}
                    </div>
